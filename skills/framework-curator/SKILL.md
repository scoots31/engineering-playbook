...
11. **Skill names are never user-facing** — the action is surfaced in plain language, never the skill name. "Plan out the build sequence" not "run prd-to-plan." "Start building" not "start solo-build." Internal routing stays internal. Quote blocks and prompts shown to the solo must never contain a skill name.
12. **No abbreviations in user-facing output** — SBF, MCP, and any code-identifier-style term never appear in output shown to the solo. "The framework" replaces "SBF" everywhere in user-facing context.
